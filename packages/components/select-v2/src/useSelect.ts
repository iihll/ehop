...
import { ArrowUp } from '@ehop/icons-vue'
...
const useSelect = (props: ExtractPropTypes<typeof SelectProps>, emit) => {
  ...
  const popper = ref<InstanceType<typeof EhTooltip> | null>(null)
  ...
  const currentPlaceholder = computed(() => {
    const _placeholder = props.placeholder || t('eh.select.placeholder')
  ...
  const handleBlur = (event: FocusEvent) => {
    ...
    // https://github.com/EhemeFE/element/pull/10822
    return nextTick(() => {
      inputRef.value?.blur?.()
