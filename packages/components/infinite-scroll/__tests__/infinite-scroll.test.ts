...
import makeScroll from '@ehopils/make-scroll'
import tick from '@ehopils/tick'
import defineGetter from '@ehop/test-utils/define-getter'
...
describe('InfiniteScroll', () => {
  ...
  test('callback will not be triggered infinitely', async () => {
    ...
    wrapper.vm.$refs.ulRef.EhInfiniteScroll.instance.count++
    await nextTick()
    expect(countListItem(wrapper)).toBe(INITIAL_VALUE)
  })
})
