<template>
  <div :id="range ? inputId : undefined" ref="sliderWrapper" :class="sliderKls" :role="range ? 'group' : undefined"
    ...
    <eh-input-number v-if="showInput && !range" ref="input" :model-value="firstValue" :class="ns.e('input')" :step="step"
      :disabled="sliderDisabled" :controls="showInputControls" :min="min" :max="max" :debounce="debounce"
...
import { useFormItemInputId, useFormSize } from '@ehop/components/form'
...
const firstButtonLabel = computed<string>(() => {
  if (props.range) {
    return props.rangeStartLabel || t('eh.slider.defaultRangeStartLabel')
  } else {
...
const secondButtonLabel = computed<string>(() => {
  return props.rangeEndLabel || t('eh.slider.defaultRangeEndLabel')
})
