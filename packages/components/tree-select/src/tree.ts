...
import { isFunction } from '@ehop/utils'
...
import type EhSelect from '@ehop/components/select'
import type Node from '@ehop/components/tree/src/model/node'
import type { TreeNodeData } from '@ehop/components/tree/src/tree.type'
...
export const useTree = (
  ...
  return {
    ...pick(toRefs(props), Object.keys(EhTree.props)),
    ...attrs,
    nodeKey: key,
    ...
    onCheck: (data, params) => {
      ...
      // only can select leaf node
        ...
        if (props.multiple) {
          emit(
            UPDATE_MODEL_EVENT,
            (tree.value as InstanceType<typeof EhTree>).getCheckedKeys(true)
          )
        } else {
