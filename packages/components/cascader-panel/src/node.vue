<template>
  <li :id="`${menuId}-${node.uid}`" role="menuitem" :aria-haspopup="!isLeaf" :aria-owns="isLeaf ? null : menuId"
    ...
    <!-- prefix -->
    <eh-checkbox v-if="multiple" :model-value="node.checked" :indeterminate="node.indeterminate" :disabled="isDisabled"
      @click.stop @update:model-value="handleSelectCheck" />
    <eh-radio v-else-if="checkStrictly" :model-value="checkedNodeId" :label="node.uid" :disabled="isDisabled"
    ...
    <eh-icon v-else-if="isLeaf && node.checked" :class="ns.e('prefix')">
      <check />
    </eh-icon>
    ...
    <template v-if="!isLeaf">
      <eh-icon v-if="node.loading" :class="[ns.is('loading'), ns.e('postfix')]">
        <loading />
      </eh-icon>
      <eh-icon v-else :class="['arrow-right', ns.e('postfix')]">
        <arrow-right />
      </eh-icon>
    </template>
  </li>
...
import EhRadio from '@ehop/components/radio'
import EhIcon from '@ehop/components/icon'
import { useNamespace } from '@ehop/hooks'
import { ArrowRight, Check, Loading } from '@ehop/icons-vue'
