<template>
  <div :class="[ns.b('spinner'), { 'has-seconds': showSeconds }]">
    <template v-if="!arrowControl">
      <eh-scrollbar v-for="item in spinnerItems" :key="item" :ref="(scrollbar: unknown) => setRef(scrollbar as any, item)"
        :class="ns.be('spinner', 'wrapper')" wrap-style="max-height: inherit;" :view-class="ns.be('spinner', 'list')"
        noresize tag="ul" @mouseenter="emitSelectRange(item)" @mousemove="adjustCurrentSpinner(item)">
    ...
    <template v-if="arrowControl">
      <div v-for="item in spinnerItems" :key="item" :class="[ns.be('spinner', 'wrapper'), ns.is('arrow')]"
        ...
        <eh-icon v-repeat-click="onDecrement" :class="['arrow-up', ns.be('spinner', 'arrow')]">
          <arrow-up />
        </eh-icon>
        <eh-icon v-repeat-click="onIncrement" :class="['arrow-down', ns.be('spinner', 'arrow')]">
          <arrow-down />
        </eh-icon>
...
import { getStyle } from '@ehop/utils'
