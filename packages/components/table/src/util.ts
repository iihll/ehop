...
import { useDelayedToggle } from '@ehop/hooks'
import type { PopperInstance } from '@ehop/components/popper'
import type { Nullable } from '@ehop/utils'
import type { EhTooltipProps } from '@ehop/components/tooltip'
...
export function getRowIdentity<T>(row: T,
  rowKey: string | ((row: T) => any)): string {
  if (!row)
...
export function isFixedColumn<T>(index: number,
...
export function getFixedColumnsClass<T>(namespace: string,
  index: number,
  fixed: string | boolean,
  store: any,
  realColumns?: TableColumnCtx<T>[],
...
export function getFixedColumnOffset<T>(index: number,
  fixed: string | boolean,
  store: any,
  realColumns?: TableColumnCtx<T>[]) {
  const {
