...
If Ehop is fully imported, it will add the following global methods for `app.config.globalProperties`: `$msgbox`, `$alert`, `$confirm` and `$prompt`. So in a Vue instance you can call `MessageBox` like what we did in this page. The parameters are:
...
EhMessageBox({}, appContext)
// or if you are using variants
EhMessageBox.alert('Hello world!', 'Title', {}, appContext)
```
