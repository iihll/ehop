...
Ehop uses BEM-styled CSS so that you can override styles easily. But if
...
- Full import: [ehop-vite-starter](https://github.com/ehop/ehop-vite-starter)
- On demand: [unplugin-ehop/examples/vite](https://github.com/ehop/unplugin-ehop)

### By SCSS variables

`theme-chalk` is written in SCSS.
You can find SCSS variables in [`packages/theme-chalk/src/common/var.scss`](https://github.com/ehop/ehop/blob/dev/packages/theme-chalk/src/common/var.scss).
...
In the future, we will write documentation for variables that can be customized for each component. You can also directly checkout the source [var.scss](https://github.com/ehop/ehop/blob/dev/packages/theme-chalk/src/common/var.scss).
...
If your project also uses SCSS, you can directly change Ehop style variables. Create a new style file, e.g. `styles/element/index.scss`:
...
@forward 'ehop/theme-chalk/src/common/var.scss' with (
  $colors: (
    'primary': (
      'base': green,
    ),
...
// @use "ehop/theme-chalk/src/index.scss" as *;
```
...
Import `element/index.scss` before scss of ehop to avoid the problem of sass mixed variables, because we need generate light-x by your custom variables.

:::

Create a `element/index.scss` to combine your variables and variables of ehop. (If you import them in ts, they will not be combined.)

:::tip

In addition, you should distinguish your scss from the element variable scss.
If they are mixed together, each hot update of `ehop` needs to compile a large number of scss files, resulting in slow speed.
...
import ElementPlus from 'unplugin-ehop/webpack'
...
```html
<el-tag style="--eh-tag-bg-color: red">Tag</el-tag>
```
...
// get css var
getComputedStyle(el).getPropertyValue(`--eh-color-primary`)

// set css var
el.style['--eh-color-primary'] = 'red'
```
