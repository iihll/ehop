...
import { errorAndExit, getWorkspacePackages } from '@ehop/build-utils'
...
async function main() {
  ...
  consola.debug(chalk.yellow(`Updating package.json for ehop`))

  const pkgs = Object.fromEntries(
    (await getWorkspacePackages()).map((pkg) => [pkg.manifest.name!, pkg])
  )
  const elementPlus = pkgs['ehop'] || pkgs['@ehop/nightly']
  const eslintConfig = pkgs['@ehop/eslint-config']
  const metadata = pkgs['@ehop/metadata']
